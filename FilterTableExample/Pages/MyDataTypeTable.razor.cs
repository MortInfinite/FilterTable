...
using FilterTableExample.Shared;
using Microsoft.AspNetCore.Components;
using FilterTable;

namespace FilterTableExample.Pages
{
	public partial class MyDataTypeTable
	{
		#region Methods
		/// <summary>
		...
		/// <summary>
		/// Configures default filter values and persists the filters to the query string.
		/// </summary>
		...
		protected override async Task OnAfterRenderAsync(bool firstRender)
		{
			...
			if(Table != null)
			{
				Table.DefaultDataFilterOperators	[nameof(MyDataType.ExpirationDate)]	= FilterOperators.GreaterThan;
				Table.DefaultDataFilterValues		[nameof(MyDataType.ExpirationDate)]	= new DateTime(2000, 1, 1).ToString("yyyy-MM-dd");
			}
		...
		protected virtual async Task<DataTable<MyDataType>.FilteredResult> GetData(IEnumerable<Expression<Func<MyDataType, bool>>> filters, string sortLabel, bool sortAscending, int skip, int maxCount, CancellationToken cancellationToken)
		{ 
			Data.MyDataTypeService.QueryResult queryResult = await MyDataTypeService.GetData(filters, sortLabel, sortAscending, skip, maxCount, cancellationToken);
		...
		/// </summary>
		public ISet<MyDataType> SelectedItems
		{
			get; 
		} = new HashSet<MyDataType>();
		...
		/// <summary>
		/// Table containing <see cref="MyDataType"/> items.
		/// </summary>
		protected DataTable<MyDataType>? Table
		{ 
			get; 
			set;
		}
		...
		/// </summary>
		protected virtual Dictionary<string, Expression<Func<MyDataType, bool>>> Filters
		{
			get; 
			set;
		} = new Dictionary<string, Expression<Func<MyDataType, bool>>>();
